...
1. **データベース統合**: Cloudflare D1を使用してデータを永続化
   ...
   - シフトテーブル
   - 配置基準テーブル
2. **シフト編集機能**: ドラッグ&ドロップでのシフト変更
3. **配置基準チェック**: リアルタイムでの配置基準違反検出

### 優先度: 中
4. **AIコマンド実装**: 自然言語でのシフト操作
5. **CSV機能**: シフトデータのインポート/エクスポート
6. **認証機能**: 管理者ログイン機能
7. **通知機能**: 配置基準違反の通知

### 優先度: 低
8. **レスポンシブ対応の強化**: モバイル表示の最適化
9. **活動履歴**: スタッフの勤務履歴表示
10. **レポート機能**: 月次・年次のレポート生成
